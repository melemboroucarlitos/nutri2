...
const Login: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const test = await axios.post('/auth/login', {username, password})
            console.log(test)
            router.push('/')
        } catch (error) {
            console.log(error)
    ...
    return (
        ...
        <div className="flex flex-col items-center mt-4">
            ...
            <span className="text-sm font-hairline text-gray-800">Dont have an account? <a className="text-green-700" href="/login">Sign Up</a></span>
            <form className="flex flex-col p-4 mt-5 bg-green-200 rounded-lg shadow-md" onSubmit={handleSubmit}>
                ...
                <button className="p-1 mt-4 mb-1 uppercase bg-green-700 border border-green-200 rounded-md shadow-lg focus:outline-none focus:border-green-900">Log In</button>
            </form>
        </div>
    </div>
    )
